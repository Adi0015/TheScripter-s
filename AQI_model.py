...
def AQI_forecast(district):
    data = pd.read_csv("../TheScripter-s/Preprocessing/AQI/AQI.csv")
    ...
    temp = pd.DataFrame({'Date': pd.date_range(pred_start_date, pred_end_date, freq='D'),
                        'Location': district, 'AQI': 0, 'NO2': 0, 'SO2': 0, 'PM2.5': 0, 'PM10': 0})
    ...
    model_aqi = SARIMAX(train_data['AQI'], order=(
        1, 1, 1), seasonal_order=(1, 1, 1, 12))
    results_aqi = model_aqi.fit()

    model_no2 = SARIMAX(train_data['NO2'], order=(
        1, 1, 1), seasonal_order=(1, 1, 1, 12))
    results_no2 = model_no2.fit()

    model_so2 = SARIMAX(train_data['SO2'], order=(
        1, 1, 1), seasonal_order=(1, 1, 1, 12))
    results_so2 = model_so2.fit()

    model_pm25 = SARIMAX(train_data['PM2.5'], order=(
        1, 1, 1), seasonal_order=(1, 1, 1, 12))
    results_pm25 = model_pm25.fit()

    model_pm10 = SARIMAX(train_data['PM10'], order=(
        1, 1, 1), seasonal_order=(1, 1, 1, 12))
    results_pm10 = model_pm10.fit()

    aqi_predictions = results_aqi.predict(
        start=start_date, end=end_date, dynamic=False)

    no2_predictions = results_no2.predict(
        start=start_date, end=end_date, dynamic=False)

    so2_predictions = results_so2.predict(
        start=start_date, end=end_date, dynamic=False)

    pm25_predictions = results_pm25.predict(
        start=start_date, end=end_date, dynamic=False)

    pm10_predictions = results_pm10.predict(
        start=start_date, end=end_date, dynamic=False)
...
def AQIrunner():
    all_predictions = pd.DataFrame(
        columns=['Date', 'Location', 'AQI', 'NO2', 'SO2', 'PM2.5', 'PM10'])
    for district in districts:
        predictions = AQI_forecast(district)
        predictions = predictions.drop_duplicates()
        all_predictions = pd.concat(
            [all_predictions, predictions], axis=0, ignore_index=True)

    all_predictions = all_predictions[all_predictions['Date'] != str(
        date.today())]
    all_predictions.to_csv("../Data/AQI_Data/AQI_forecast.csv", index=False)
...
def AQI_dataConsistence():
    ...
    data.to_csv("../TheScripter-s/Preprocessing/AQI/AQI.csv", index=False)
